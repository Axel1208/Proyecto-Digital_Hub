...
const EquipmentManagement = () => {
  ...
  const estadoColor = (estado) => ({
    disponible: '#4ade80', asignado: '#facc15', 'dañado': '#f87171', 'en reparacion': '#fb923c'
  }[estado] || '#c9a8ff');

  const portatilesFiltrados = portatiles.filter(p => {
    const buscar = filtros.buscar.toLowerCase();
    return (
      (!buscar || p.num_serie.toLowerCase().includes(buscar) || p.marca.toLowerCase().includes(buscar) || p.modelo.toLowerCase().includes(buscar)) &&
      ...
      <main className="equipment-main">
        <div className="equipment-header">
          <div>
            ...
            <p className="equipment-subtitle">Total de equipos registrados: <span>{portatiles.length}</span></p>
          </div>
        ...
        <div className="stats-grid">
          <div className="stat-card">
            <div className="stat-label">Total Portátiles</div>
                ...
                <tr key={p.id_portatil}>
                  ...
                  <td>
                    <div className="action-buttons">
                      <button className="action-btn view" onClick={() => abrirVer(p)} title="Ver"><IconEye size={16} /></button>
                      <button className="action-btn edit" onClick={() => abrirEditar(p)} title="Editar"><IconPencil size={16} /></button>
                      <button className="action-btn delete" onClick={() => handleEliminar(p.id_portatil)} title="Eliminar"><IconTrash size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {showModal && (
          <div className="modal-overlay" onClick={() => setShowModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <h2 className="modal-title">Añadir nuevo portátil</h2>
              {error && <p className="table-error">{error}</p>}
              <form onSubmit={handleSubmit}>
                <div className="form-group"><label>Número de serie</label>
                  <input type="text" name="num_serie" value={formData.num_serie} onChange={(e) => setFormData({...formData, num_serie: e.target.value})} required />
                </div>
